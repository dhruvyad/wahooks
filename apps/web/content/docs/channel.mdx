...
- A WAHooks [API token](/docs/concepts/api-tokens)
...
Install the package globally and add it to your MCP config:
...
Add the WAHooks marketplace, then install the plugin:

```bash
/plugin marketplace add dhruvyad/wahooks
/plugin install wahooks@dhruvyad/wahooks
```
...
This stores your key in `~/.claude/channels/wahooks/.env`. You only need to do this once.

### 2. Set up a webhook

Create a webhook on your WAHooks connection that points to the channel's local server:

```bash
wahooks webhooks create CONNECTION_ID http://localhost:8790/webhook "message,message.any"
```

Or via the [dashboard](https://wahooks.com/connections) — add a webhook with URL `http://localhost:8790/webhook`.
