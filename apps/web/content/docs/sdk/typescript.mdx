...
const chats = await client.getChats(connectionId, { limit: 50, unreadOnly: false });
console.log(chats[0].name, chats[0].lastMessage?.body);

// Page through a chat's messages (newest-first)
let page = await client.getMessages(connectionId, chatId, { limit: 50 });
console.log(page.messages);
while (page.nextBefore) {
  page = await client.getMessages(connectionId, chatId, { before: page.nextBefore });
}
// page.historyStartsAt is set (oldest timestamp) once history is exhausted

// A single message by id (chatId required)
const msg = await client.getMessage(connectionId, messageId, chatId);

// Download a message's media (same Bearer auth as other calls)
const url = client.getMediaUrl(connectionId, messageId, chatId);
const res = await fetch(url, { headers: { Authorization: `Bearer ${apiKey}` } });

// Contacts (people)
const contacts = await client.getContacts(connectionId);

// Edit a message you sent — great for live status updates.
// sendMessage returns the message id.
const sent = await client.sendMessage(connectionId, chatId, 'Working on it…');
await client.editMessage(connectionId, sent.id, chatId, 'Done ✅ — here is the result');
