"""WAHooks MCP Server.

Usage:
  # As stdio server (for Claude Desktop / Claude Code):
  WAHOOKS_API_KEY=wh_... wahooks-mcp

  # As HTTP server:
  WAHOOKS_API_KEY=wh_... wahooks-mcp --http --port 8000

Claude Desktop config (~/.claude/claude_desktop_config.json):
  {
    "mcpServers": {
      "wahooks": {
        "command": "wahooks-mcp",
        "env": { "WAHOOKS_API_KEY": "wh_..." }
      }
    }
  }
"""

import os
import sys
from contextlib import asynccontextmanager

import httpx
from fastmcp import FastMCP

API_BASE = os.environ.get("WAHOOKS_API_URL", "https://api.wahooks.com")

_http: httpx.AsyncClient | None = None


def _get_client() -> httpx.AsyncClient:
    global _http
    if _http is None or _http.is_closed:
        api_key = os.environ.get("WAHOOKS_API_KEY", "")
        if not api_key:
            print("ERROR: Set WAHOOKS_API_KEY environment variable", file=sys.stderr)
            sys.exit(1)
        _http = httpx.AsyncClient(
            base_url=f"{API_BASE}/api",
            headers={
                "Authorization": f"Bearer {api_key}",
                "Content-Type": "application/json",
            },
            timeout=30.0,
        )
    return _http


mcp = FastMCP(
    name="WAHooks",
    instructions=(
        "WAHooks MCP server for managing WhatsApp connections, "
        "sending/receiving messages, and configuring webhooks. "
        "Use list_connections to see active connections, "
        "send_message to send WhatsApp messages, "
        "and create_webhook to receive message notifications."
    ),
)


# ---------------------------------------------------------------------------
# Helpers
# ---------------------------------------------------------------------------

async def _get(path: str) -> dict | list:
    r = await _get_client().get(path)
    r.raise_for_status()
    return r.json()


async def _post(path: str, json: dict | None = None) -> dict:
    r = await _get_client().post(path, json=json)
    r.raise_for_status()
    return r.json()


async def _put(path: str, json: dict) -> dict:
    r = await _get_client().put(path, json=json)
    r.raise_for_status()
    return r.json()


async def _delete(path: str) -> dict:
    r = await _get_client().delete(path)
    r.raise_for_status()
    return r.json()


# ---------------------------------------------------------------------------
# Connection tools
# ---------------------------------------------------------------------------

@mcp.tool
async def list_connections() -> list[dict]:
    """List all active WhatsApp connections."""
    return await _get("/connections")


@mcp.tool
async def create_connection() -> dict:
    """Create a new WhatsApp connection. Returns connection ID and status.
    After creation, use get_qr to get the QR code for linking."""
    return await _post("/connections")


@mcp.tool
async def get_connection(connection_id: str) -> dict:
    """Get details of a specific connection."""
    return await _get(f"/connections/{connection_id}")


@mcp.tool
async def delete_connection(connection_id: str) -> dict:
    """Delete a WhatsApp connection. This stops the session and unlinks the number."""
    return await _delete(f"/connections/{connection_id}")


@mcp.tool
async def restart_connection(connection_id: str) -> dict:
    """Restart a WhatsApp connection. The user will need to scan QR again."""
    return await _post(f"/connections/{connection_id}/restart")


@mcp.tool
async def get_qr(connection_id: str) -> dict:
    """Get the QR code for linking a WhatsApp account. Returns base64 PNG.
    The user must scan this with WhatsApp > Settings > Linked Devices."""
    return await _get(f"/connections/{connection_id}/qr")


@mcp.tool
async def get_chats(connection_id: str) -> list[dict]:
    """Get recent WhatsApp chats for a connection. Returns chat IDs and names."""
    return await _get(f"/connections/{connection_id}/chats")


@mcp.tool
async def get_profile(connection_id: str) -> dict:
    """Get the WhatsApp profile info (phone number, display name) for a connection."""
    return await _get(f"/connections/{connection_id}/me")


@mcp.tool
async def send_message(connection_id: str, chat_id: str, text: str) -> dict:
    """Send a WhatsApp message.

    Args:
        connection_id: The connection to send from.
        chat_id: Recipient in WhatsApp format (e.g. '1234567890@s.whatsapp.net'
                 for individuals, or 'id@g.us' for groups).
        text: Message text to send.
    """
    return await _post(
        f"/connections/{connection_id}/send",
        json={"chatId": chat_id, "text": text},
    )


# ---------------------------------------------------------------------------
# Webhook tools
# ---------------------------------------------------------------------------

@mcp.tool
async def list_webhooks(connection_id: str) -> list[dict]:
    """List webhook configurations for a connection."""
    return await _get(f"/connections/{connection_id}/webhooks")


@mcp.tool
async def create_webhook(
    connection_id: str,
    url: str,
    events: list[str] | None = None,
) -> dict:
    """Create a webhook to receive WhatsApp events at a URL.

    Args:
        connection_id: The connection to attach the webhook to.
        url: The URL to receive webhook POST requests.
        events: Event types to receive (default: all). Options: 'message',
                'message.any', 'message.ack', 'session.status', 'presence.update'.
    """
    return await _post(
        f"/connections/{connection_id}/webhooks",
        json={"url": url, "events": events or ["*"]},
    )


@mcp.tool
async def update_webhook(
    webhook_id: str,
    url: str | None = None,
    events: list[str] | None = None,
    active: bool | None = None,
) -> dict:
    """Update a webhook configuration.

    Args:
        webhook_id: The webhook to update.
        url: New URL (optional).
        events: New event filter (optional).
        active: Enable or disable the webhook (optional).
    """
    body: dict = {}
    if url is not None:
        body["url"] = url
    if events is not None:
        body["events"] = events
    if active is not None:
        body["active"] = active
    return await _put(f"/webhooks/{webhook_id}", json=body)


@mcp.tool
async def delete_webhook(webhook_id: str) -> dict:
    """Delete a webhook configuration."""
    return await _delete(f"/webhooks/{webhook_id}")


@mcp.tool
async def get_webhook_logs(webhook_id: str) -> list[dict]:
    """Get delivery logs for a webhook. Shows event type, status, and payload."""
    return await _get(f"/webhooks/{webhook_id}/logs")


@mcp.tool
async def test_webhook(webhook_id: str) -> dict:
    """Send a test event to a webhook to verify it's working."""
    return await _post(f"/webhooks/{webhook_id}/test")


# ---------------------------------------------------------------------------
# Token tools
# ---------------------------------------------------------------------------

@mcp.tool
async def list_tokens() -> list[dict]:
    """List active API tokens."""
    return await _get("/tokens")


@mcp.tool
async def create_token(name: str) -> dict:
    """Create a new API token. The raw token is shown only once — save it immediately.

    Args:
        name: A descriptive name for the token (e.g. 'my-app', 'production').
    """
    return await _post("/tokens", json={"name": name})


@mcp.tool
async def revoke_token(token_id: str) -> dict:
    """Revoke an API token. It will immediately stop working."""
    return await _delete(f"/tokens/{token_id}")


# ---------------------------------------------------------------------------
# Entry point
# ---------------------------------------------------------------------------

def main():
    import argparse

    parser = argparse.ArgumentParser(description="WAHooks MCP Server")
    parser.add_argument("--http", action="store_true", help="Run as HTTP server")
    parser.add_argument("--port", type=int, default=8000, help="HTTP port (default: 8000)")
    parser.add_argument("--host", default="0.0.0.0", help="HTTP host (default: 0.0.0.0)")
    args = parser.parse_args()

    if args.http:
        mcp.run(transport="http", host=args.host, port=args.port)
    else:
        mcp.run(transport="stdio")


if __name__ == "__main__":
    main()
